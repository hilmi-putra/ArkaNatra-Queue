        ...
        <div
            ...
            <div class="absolute top-2 end-2">
                <button type="button"
                    ...
                    aria-label="Close" data-hs-overlay="#hs-delete-work-type-modal">
            ...
            <div class="p-4 sm:p-10 overflow-y-auto">
                <div class="flex gap-x-4 md:gap-x-7">
                    ...
                    <div class="grow">
                        <h3 id="hs-delete-work-type-modal-label"
                            class="mb-2 text-xl font-bold text-gray-800 dark:text-neutral-200">
                            Konfirmasi Penghapusan Work Type
                        </h3>
                        <p class="text-gray-500 dark:text-neutral-500 mb-4">
                            Apakah Anda yakin ingin menghapus data work type berikut?
                        </p>

                        <div class="text-sm bg-gray-50 dark:bg-neutral-800 p-4 rounded-lg mb-4 space-y-3">
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600 dark:text-neutral-400 font-medium">Work Type:</span>
                                <span class="font-semibold text-gray-800 dark:text-neutral-200"
                                    id="modal-work-type-name">—</span>
                            </div>
                            <div class="flex justify-between items-center">
                                ...
                                <span class="font-medium text-gray-800 dark:text-neutral-200"
                                    id="modal-work-type-division">—</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600 dark:text-neutral-400 font-medium">Estimasi Reguler:</span>
                                <span class="font-medium text-blue-600 dark:text-blue-400"
                                    id="modal-work-type-regular">—</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600 dark:text-neutral-400 font-medium">Hari Tambahan:</span>
                                <span class="font-medium text-gray-800 dark:text-neutral-200"
                                    id="modal-work-type-extra">—</span>
                            </div>
                        </div>
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="text-sm">
                                    ...
                                    <p class="text-yellow-700 dark:text-yellow-500 mt-1">
                                        Semua data terkait work type ini akan dihapus secara permanen.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <form id="delete-work-type-form" method="POST" action="">
                @csrf
                @method('DELETE')
                <input type="hidden" name="work_type_id" id="work-type-id-input">
...
<script>
    window.addEventListener('load', () => {
        ...
        const deleteButtons = document.querySelectorAll('.delete-work-type-btn');
        const modalWorkTypeName = document.getElementById('modal-work-type-name');
        const modalWorkTypeDivision = document.getElementById('modal-work-type-division');
        const modalWorkTypeRegular = document.getElementById('modal-work-type-regular');
        const modalWorkTypeExtra = document.getElementById('modal-work-type-extra');
        const deleteForm = document.getElementById('delete-work-type-form');
        const workTypeIdInput = document.getElementById('work-type-id-input');

        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                const workTypeId = button.getAttribute('data-work-type-id');
                const workTypeName = button.getAttribute('data-work-type-name');
                const workTypeDivision = button.getAttribute('data-work-type-division');
                const workTypeRegular = button.getAttribute('data-work-type-regular');
                const workTypeExtra = button.getAttribute('data-work-type-extra');

                // Populate modal with work type data
                modalWorkTypeName.textContent = workTypeName;
                modalWorkTypeDivision.textContent = workTypeDivision;
                modalWorkTypeRegular.textContent = workTypeRegular + ' hari';
                modalWorkTypeExtra.textContent = workTypeExtra + ' hari';
                workTypeIdInput.value = workTypeId;

                // Set the form action URL
                let baseUrl = "{{ url('admin/work-types') }}";
                deleteForm.action = `${baseUrl}/${workTypeId}`;
            });
        });
    });
</script>
